...
Keep only BING (positive/negataive) and AFIN (-5/5 scale) lexicons
```{r}
sentiments <- tidytext::sentiments %>%
  filter(lexicon %in% c("bing", "AFINN", "nrc"),
         (lexicon == "nrc" & sentiment %in% c("positive", "negative")),
         !(grepl("envious", word) & sentiment == "positive")) %>%
  mutate(sentiment_score = ifelse(is.na(sentiment), score, sentiment),
