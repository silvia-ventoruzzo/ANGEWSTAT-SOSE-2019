...
distribution_plots[["artist"]] <- songdata %>%
  ...
  geom_bar(aes(x = reorder(artist, occurences), y = occurences, fill = artist), stat = "identity") +
  guides (fill = FALSE) +
  scale_y_continuous(labels = scales::percent_format()) +
...
distribution_plots[["genre"]] <- songdata %>%
  ...
  geom_bar(aes(x = reorder(genre, occurences), y = occurences, fill = genre), stat = "identity") +
  guides(fill = FALSE) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(axis.title.y = element_text(size = 12),
        axis.title.x = element_blank()) +
...
distribution_plots[["year"]] <- songdata %>%
  ...
  geom_bar(aes(x = year, y = occurences, fill = factor(year)), stat = "identity") +
  guides(fill = FALSE) +
  ...
  coord_flip()
grid.newpage()
grid::grid.draw(rbind(ggplotGrob(distribution_plots[["artist"]]), 
                      ggplotGrob(distribution_plots[["genre"]]), 
                      ggplotGrob(distribution_plots[["year"]]), 
                      size = "first"))
```
...
songdata_words %>%
  count(genre, song) %>%
  ggplot() +
  geom_boxplot(aes(x = factor(0), y = n, fill = genre, color = genre), alpha = 0.5) +
...
bing_and_nrc <- bind_rows(
  ...
  songdata_words %>% 
    ...
    mutate(method = "NRC")) %>%
  count(method, genre, artist, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
  mutate(sentiment = positive - negative)

bind_rows(afinn, 
          bing_and_nrc) %>%
  ggplot(aes(artist, sentiment, fill = method)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  facet_grid(rows = vars(genre), cols = vars(method), scales = "free", switch = "y") +
  theme(axis.title.x = element_text(size = 12),
        axis.title.y = element_blank())
```
...
rm("DTM_test", "DTM_train", "fitted", "test_data", "train_data", "i", "topics", "customer_stopwords")
```
...
```{r}
topic_numbers %>%
  inner_join(perplexity_df, by = "topics") %>%
...
#                                       topics  = seq(2, length(songdata_dtm$dimnames$Docs), by = 1),
#                                       metrics = c("Griffiths2004", "CaoJuan2009", "Arun2010", "Deveaud2014"),
#                                       method  = "VEM",
#                                       control = list(seed = 4324663),
#                                       verbose = TRUE)
...
songdata_beta[["3"]] %>%
  ...
  mutate(sentiment = "Weighted average sentiment") %>%
  ggplot(aes(reorder(topic, score), score, fill = topic)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  theme(axis.title.y = element_blank(),
        axis.text.x  = element_text(size = 14),
...
for (k in 2:(length(songdata_gamma)+1)) {
  ...
  plot <- year_gamma %>%
    ...
    geom_boxplot(alpha = 0.5) +
    labs(y        = "probability",
         title    = "Topic probabilities per year") +
    theme(axis.title.x = element_blank()) +
    guides(fill = FALSE, color = FALSE) +
    facet_wrap(~year)
...
for (k in 2:(length(songdata_gamma)+1)) {
  ...
  for (g in unique(genre_gamma$genre)) {
...
for (k in 2:(length(songdata_gamma)+1)) {
  ...
  plot <- songdata_gamma[[k-1]] %>%
    ...
    dplyr::select(-year, -text) %>%
    mutate(topic          = paste0("topic", topic)) %>%
    ggplot(aes(artist, gamma, fill = topic)) +
    geom_col(show.legend = FALSE) +
    coord_flip() +
    facet_grid(rows = vars(genre), cols = vars(topic), scales = "free_y", switch = "y") +
    theme(axis.title.x = element_text(size = 12),
          axis.title.y = element_blank())
